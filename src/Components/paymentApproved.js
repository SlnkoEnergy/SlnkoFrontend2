...
function PaymentRequest() {
  ...
  const AccountMatch = ({ payment, onValidationChange }) => {
    ...
    const [utr, setUtr] = React.useState(""); // UTR field state
    const [error, setError] = React.useState(null);
    const [isMatched, setIsMatched] = React.useState(payment.acc_match === "matched");
    const [utrSubmitted, setUtrSubmitted] = React.useState(false); // Track if UTR is submitted
    ...
    const handleUtrSubmit = (e) => {
      ...
      setUtrSubmitted(true); // Disable UTR field after submission
    ...
    return (
      <Dropdown>
        ...
        <Menu size="sm" sx={{ minWidth: 250, padding: 1 }}>
          <form onSubmit={isMatched && !utrSubmitted ? handleUtrSubmit : handleMatchSubmit}>
            <Input
            ...
            {isMatched && !utrSubmitted && (
              <Input
                ...
                sx={{ width: "100%", marginBottom: "1rem" }}
                disabled={utrSubmitted} // Disable if UTR is submitted
              />
            )}
            ...
            <MenuItem>
              <Button
                ...
                sx={{ width: "100%" }}
              >
                {isMatched && !utrSubmitted ? "Submit" : "Match"}
              </Button>
            </MenuItem>
          </form>
        </Menu>
